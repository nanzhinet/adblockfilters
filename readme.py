...
class ReadMe(object):
    ...
    def __subscribeLink(self, fileName:str, url:str=None):
        ...
        if url:
            ...
            link += " [原始链接](https://raw.githubusercontent.com/nanzhinet/adblockfilters/main/rules/%s) |"%(fileName)
        
        for i in range(1, len(self.proxyList)):
            proxy = self.proxyList[i]
            if proxy.startswith("https://gcore.jsdelivr.net/"):
                link += " [加速链接%d](%s/nanzhinet/adblockfilters@main/rules/%s) |"%(i, proxy, fileName)
            else:
                link += " [加速链接%d](%s/https://raw.githubusercontent.com/nanzhinet/adblockfilters/main/rules/%s) |"%(i, proxy, fileName)
    ...
    def regenerate(self):
        ...
        with open(self.filename, 'a') as f:
            ...
            f.write("5. 不再引用[NEO DEV HOST](https://github.com/neodevpro/neodevhost/blob/master/lite_adblocker)，原项目[已停止维护]。\n")
            ...
            f.write("3. 移除[ADgk](https://raw.githubusercontent.com/banbendalao/ADgk/master/ADgk.txt)、[SmartTV Blocklist](https://raw.githubusercontent.com/Perflyst/PiHoleBlocklist/master/SmartTV-AGH.txt)、[1024 hosts](https://raw.githubusercontent.com/Goooler/1024_hosts/master/hosts)、[ad-wars hosts](https://raw.githubusercontent.com/jdlingyu/ad-wars/master/hosts)规则长时间未更新。\n")
            f.write("4. 不再引用[NEO DEV HOST](https://github.com/neodevpro/neodevhost/blob/master/lite_adblocker)，原项目[已停止维护]。\n")
            f.write("5. 不再引用[xinggsf rule](https://raw.githubusercontent.com/xinggsf/Adblock-Plus-Rule/master/rule.txt)、[1Hosts (Lite)](https://raw.githubusercontent.com/badmojr/1Hosts/master/Lite/adblock.txt)，误杀较多。\n")
            f.write("\n")
            ...
            f.write("[![Star History Chart](https://api.star-history.com/svg?repos=nanzhinet/adblockfilters&type=Date)](https://star-history.com/#nanzhinet/adblockfilters&Date)\n")
            '''
            f.write("\n")
            f.write("## 以下是广告\n")
            f.write('感兴趣的可以看下，DartNode 免费 VPS, [点击申请]()\n')
            f.write('[![Powered by DartNode](https://png)](https:// "Powered by DartNode - Free VPS for Open Source")\n')
            '''
